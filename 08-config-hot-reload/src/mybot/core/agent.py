"""Agent and AgentSession for step 08 with event-driven and web tools support."""
...
class Agent:
    ...
    def load_session(self, session_id: str) -> "AgentSession":
        ...
        self.context.history_store.set_active_session(self.agent_def.id, session_id)
    ...
    def resume_active_session(self) -> "AgentSession | None":
        ...
        active_id = self.context.history_store.get_active_session_id(self.agent_def.id)
        if active_id:
            return self.load_session(active_id)

        latest = self.context.history_store.get_latest_session(self.agent_def.id)
        if latest is None:
            return None
