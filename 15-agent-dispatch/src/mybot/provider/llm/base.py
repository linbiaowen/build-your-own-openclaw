...
def _tool_call_from_dict(item: dict[str, Any]) -> "LLMToolCall | None":
    from mybot.core.tool_calls import normalize_tool_call_record

    norm = normalize_tool_call_record(item)
    if not norm:
        return None
    ...
    async def chat(
        ...
        **kwargs: Any,
        ...
        for tc in message.tool_calls or []:
            ...
            norm = normalize_tool_call_record(raw)
            if not norm or not norm["function"]["name"]:
                continue
            tool_calls.append(
                LLMToolCall(
                    id=norm["id"],
                    name=norm["function"]["name"],
                    arguments=norm["function"]["arguments"],
                )
            )
