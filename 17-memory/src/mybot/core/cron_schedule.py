...
def substitute_skill_templates(content: str, config: "Config") -> str:
    ...
    return substitute_workspace_templates(content, config)
...
def parse_fixed_schedule_run_at(
    schedule: str, now: datetime | None = None
) -> datetime | None:
    """Parse one-off `M H D month *` schedule into a local run time."""
    schedule = schedule.strip()
    if not is_fixed_time_schedule(schedule):
        return None
    now = now or datetime.now()
    minute, hour, day, month, _ = schedule.split()
    return datetime(
        now.year,
        int(month),
        int(day),
